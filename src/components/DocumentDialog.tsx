...
export const ReceiptDialog = ({ open, onClose, total, freight, cartItems, payments, documentType }: ReceiptDialogProps) => {
  const totalChange = payments.reduce((sum: number, p: any) => {
    if (p.type === "cash" && p.cashReceived) {
      return sum + (p.cashReceived - p.amount);
    }
  ...
  const now = new Date();
  const documentTitle = documentType === "quote" ? "ORÇAMENTO" : "CUPOM FISCAL";

...
import { Receipt, FileText, Printer, CheckCircle, Truck, Store, Phone, MapPin, AlertTriangle, Loader2, RefreshCw, Download, Copy } from "lucide-react";
...
export const DocumentDialog = ({ open, onClose, total, freight, cartItems, payments, onGenerateDocument, saleId }: DocumentDialogProps) => {
  ...
  const handleSendToFiscal = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Erro ao emitir nota fiscal. Verifique a contingência.');
      }
    } catch (err) {
      console.error('Error sending to SEFAZ:', err);
      setError('Erro de comunicação com SEFAZ');
      setIsContingency(true);
      toast.error('Erro de comunicação com SEFAZ. Verifique a contingência.');
    } finally {
      setIsSending(false);
    }
  };
                ...
                <Card className="border-red-200 bg-red-50">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-red-900">Erro na Emissão</p>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-amber-700 mt-1">
                          A nota foi encaminhada para contingência. Você poderá tentar o envio novamente dentro do prazo vigente.
                        </p>
                        <div className="mt-4">
                          <Button
                            onClick={handleRetryFiscal}

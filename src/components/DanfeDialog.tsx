...
export function DanfeDialog({
  ...
  useEffect(() => {
    ...
    if (!open || !barcodeCanvas || accessKey.length !== 44) {
      return;
    }

    JsBarcode(barcodeCanvas, accessKey, {
      format: 'CODE128',
      displayValue: false,
      height: 32,
      margin: 0,
  ...
  }, [barcodeCanvas, nfeData?.accessKey, open]);
        ...
        >
          ...
          <div className="mt-2 border-2 border-black">
            ...
            <div className="grid grid-cols-4 gap-x-2 gap-y-1 p-2 text-xs">
              <div className="col-span-2"><span className="font-semibold">Nome / Razão Social: </span>{customer?.name || '—'}</div>
              <div><span className="font-semibold">CNPJ/CPF: </span>{formatCnpj(customer?.cpf_cnpj)}</div>
              <div><span className="font-semibold">IE: </span>{customer?.inscricao_estadual || '—'}</div>
              <div className="col-span-3"><span className="font-semibold">Endereço: </span>{customer?.logradouro}, {customer?.numero}{customer?.complemento ? ` - ${customer.complemento}` : ''}</div>
              <div><span className="font-semibold">Bairro: </span>{customer?.bairro || '—'}</div>
              <div><span className="font-semibold">CEP: </span>{formatCep(customer?.cep)}</div>
              <div><span className="font-semibold">Município: </span>{customer?.municipio || '—'}</div>
              <div><span className="font-semibold">Fone: </span>{customer?.phone || '—'}</div>
              <div><span className="font-semibold">UF: </span>{customer?.uf || '—'}</div>
            </div>
          </div>

          <div className="mt-2 border-2 border-black">
            <div className="border-b border-black bg-gray-100 px-2 py-0.5"><p className="text-[11px] font-bold">FORMA DE PAGAMENTO</p></div>
            <div className="p-2 text-xs">
              {payments.length > 0
                ? payments.map((payment, index) => <span key={index} className="mr-4">{paymentLabels[payment.type] || 'Outros'}: {currency(payment.amount)}</span>)
                : '—'}
            </div>
          </div>

          <div className="mt-2 border-2 border-black">
            <div className="border-b border-black bg-gray-100 px-2 py-0.5"><p className="text-[11px] font-bold">CÁLCULO DO IMPOSTO</p></div>
            <div className="grid grid-cols-4 gap-x-2 gap-y-1 p-2 text-xs">
              ...
              <div className="col-span-4 text-right"><span className="text-sm font-black">VALOR TOTAL DA NOTA: {currency(total)}</span></div>
            </div>
          </div>

          <div className="mt-2 border-2 border-black">
            <div className="border-b border-black bg-gray-100 px-2 py-0.5"><p className="text-[11px] font-bold">TRANSPORTADOR / VOLUMES TRANSPORTADOS</p></div>
            <div className="grid grid-cols-4 gap-x-2 gap-y-1 p-2 text-xs">
              <div className="col-span-2"><span className="font-semibold">Frete por conta: </span>{freight > 0 ? 'Destinatário' : 'Sem frete'}</div>
              <div className="col-span-2"><span className="font-semibold">Valor do Frete: </span>{currency(freight)}</div>
              {motoboy && freight > 0 && <div className="col-span-4"><span className="font-semibold">Transportador: </span>{motoboy.name}</div>}
            </div>
          </div>

          <div className="mt-2 border-2 border-black">
            <div className="border-b border-black bg-gray-100 px-2 py-0.5"><p className="text-[11px] font-bold">DADOS DOS PRODUTOS / SERVIÇOS</p></div>
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-black text-[10px] font-bold">
                  <th className="p-1 text-left">CÓDIGO</th><th className="p-1 text-left">DESCRIÇÃO</th><th className="p-1 text-center">NCM</th><th className="p-1 text-center">CFOP</th><th className="p-1 text-center">UN</th><th className="p-1 text-right">QTD</th><th className="p-1 text-right">VALOR UNIT</th><th className="p-1 text-right">VALOR TOTAL</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="border-b border-gray-300 align-top">
                      ...
                      <td className="p-1">{item.product_name || item.name}{item.flavors && item.flavors.length > 0 && <span className="block text-[10px] text-gray-600">Sabores: {item.flavors.join(', ')}</span>}</td>
          ...
          <div className="mt-2 border-2 border-black">
            <div className="border-b border-black bg-gray-100 px-2 py-0.5"><p className="text-[11px] font-bold">INFORMAÇÕES COMPLEMENTARES</p></div>
            <div className="p-2 text-[11px]">
              {isHomologation ? 'NF-e EMITIDA EM AMBIENTE DE HOMOLOGAÇÃO — SEM VALOR FISCAL.' : 'Documento auxiliar da NF-e. Consulte a autenticidade no portal da SEFAZ.'}
              {nfeData?.consultationUrl && <p className="mt-1 break-all">URL de consulta: {nfeData.consultationUrl}</p>}
            </div>
          </div>
          ...
          <div className="mt-4 text-center text-[10px] text-gray-600">
            ...
            <p className="mt-1 font-semibold">{isHomologation ? 'EMITIDA EM HOMOLOGAÇÃO — SEM VALOR FISCAL' : `Autorizada sob protocolo ${nfeData?.protocol || '—'}`}</p>
          </div>
        </div>

        <DialogFooter>
          <Button variant="secondary" onClick={onClose}>Fechar</Button>
          <Button onClick={handlePrint}><Printer className="mr-2 h-4 w-4" />Imprimir DANFE</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
